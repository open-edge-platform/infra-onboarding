...
func ReadingYamlNCreatingResourse(kubeconfigPath, imgurl, imgType, filePath, fileName string) error {
	...
	for _, content := range contentInfo {
		if imgType == "prod_bkc" {
			...
			currentPath, _ = os.Getwd()
			repoPath, _ := strings.CutSuffix(currentPath, "/internal/provisioningservice/onboarding")
			...
			currentPath, _ = os.Getwd()
			azureEnvPath, _ := strings.CutSuffix(currentPath, "/internal/provisioningservice/onboarding")
