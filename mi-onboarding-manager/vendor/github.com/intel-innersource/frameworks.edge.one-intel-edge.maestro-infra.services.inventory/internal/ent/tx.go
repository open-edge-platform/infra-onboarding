...
func (tx *Tx) init() {
	...
	tx.IPAddressResource = NewIPAddressResourceClient(tx.config)
	tx.InstanceResource = NewInstanceResourceClient(tx.config)
	...
	tx.ProjectResource = NewProjectResourceClient(tx.config)
	tx.ProviderResource = NewProviderResourceClient(tx.config)
	tx.RegionResource = NewRegionResourceClient(tx.config)
