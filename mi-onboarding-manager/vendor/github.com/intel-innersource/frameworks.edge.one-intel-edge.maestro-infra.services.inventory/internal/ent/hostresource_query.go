...
func (hrq *HostResourceQuery) QueryProject() *ProjectResourceQuery {
...
func (hrq *HostResourceQuery) QueryUser() *UserResourceQuery {
	query := (&UserResourceClient{config: hrq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hrq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hrq *HostResourceQuery) WithProject(opts ...func(*ProjectResourceQuery)) *HostResourceQuery {
	query := (&ProjectResourceClient{config: hrq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (hrq *HostResourceQuery) WithUser(opts ...func(*UserResourceQuery)) *HostResourceQuery {
	query := (&UserResourceClient{config: hrq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (hrq *HostResourceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*HostResource, error) {
	...
	if hrq.withSite != nil || hrq.withProvider != nil || hrq.withProject != nil || hrq.withUser != nil || hrq.withInstance != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := hrq.withProject; query != nil {
		if err := hrq.loadProject(ctx, query, nodes, nil,
			func(n *HostResource, e *ProjectResource) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := hrq.withUser; query != nil {
		if err := hrq.loadUser(ctx, query, nodes, nil,
			func(n *HostResource, e *UserResource) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
...
func (hrq *HostResourceQuery) loadProject(ctx context.Context, query *ProjectResourceQuery, nodes []*HostResource, init func(*HostResource), assign func(*HostResource, *ProjectResource)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*HostResource)
	for i := range nodes {
		if nodes[i].host_resource_project == nil {
			continue
		}
		fk := *nodes[i].host_resource_project
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "host_resource_project" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (hrq *HostResourceQuery) loadUser(ctx context.Context, query *UserResourceQuery, nodes []*HostResource, init func(*HostResource), assign func(*HostResource, *UserResource)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*HostResource)
	for i := range nodes {
		if nodes[i].host_resource_user == nil {
			continue
		}
		fk := *nodes[i].host_resource_user
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "host_resource_user" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
